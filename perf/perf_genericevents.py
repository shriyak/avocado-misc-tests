...
class test_generic_events(Test):
    ...
    This test will read content of each file from /sys/bus/event_source/devices/cpu/events
    ...
    def setUp(self):
        ...
        for cur_ker, supported_ker in zip(kernel_Version.split(".")[:4], kernel_to_compare.split(".")[:4]):
            if cur_ker < supported_ker:
                raise self.log.warn("This test is not supported on this kernel version")
    ...
    def test(self):
        ...
        for file in os.listdir(dir):
            ...
            if event_code.split('=', 2)[1].rstrip() != val:
                nfail += 1
            self.log.info(
                'FILE in /sys/bus/event_source/devices/cpu/events is %s' % file)
